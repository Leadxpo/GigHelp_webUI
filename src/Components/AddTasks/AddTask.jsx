...
const resetForm = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const selectedCategory = categoryList.find((cat) => cat.categoryId === category);



      
      const selectedSubCategory = subCategoryList.find((sub) => sub.SubCategoryId === subCategory);
      ...
      const response = await axios.post("http://localhost:3001/task/create", formData, {
        headers: {
  ...
  return (
    ...
    <Box sx={{ p: 3, bgcolor: "#FFFFFF", minHeight: "100vh" }}>
      ...
      <Box sx={{ maxWidth: "90%", mx: "auto", p: 3, bgcolor: "white", borderRadius: 2, boxShadow: 3 }}>
  ...
  onChange={(e) => {
    ...
    const selected = categoryList.find((cat) => cat.categoryId === selectedId);
